...
func GetSymbols() ([]uint32, map[string]string, string, string) {
	...
	e := os.Remove("app/zfiles/log/instruments.csv")
	if e != nil {
		srv.InfoLogger.Println("instruments.csv deleted")
	}

	fileUrl := "http://api.kite.trade/instruments"
	err := DownloadFile("app/zfiles/log/instruments.csv", fileUrl)
	if err != nil {
	...
	// open file
	f, err := os.Open("app/zfiles/log/instruments.csv")
	if err != nil {
	...
	dat, err := ioutil.ReadFile("app/zfiles/config/trackSymbols.txt")
...
func saveFiles(data []string, fileName string) bool {
	...
	e := os.Remove("app/zfiles/log/" + fileName)
	if e != nil {
		srv.InfoLogger.Println(fileName + " deleted")
	}

	f, err := os.Create("app/zfiles/log/" + fileName)
